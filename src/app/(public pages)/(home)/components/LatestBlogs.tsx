...
const BlogCard = ({ blog }: { blog: TBlog }) => {
  return (
    <div className="w-full">
      <div className="w-full h-[250px] group mb-5 rounded-[8px] overflow-hidden ">
        <Image src={blog.thumnail} className="duration-300 group-hover:scale-110 w-full h-full object-cover" width={300} height={250} alt={blog.title} />
      </div>
      <h6 className="heading mb-2">{getSlicedText(blog.title, 50)}</h6>
      <p>{blog.excerpt ? getSlicedText(blog.excerpt, 150) : ""}</p>
      <div className="w-full flex justify-between items-center mt-3">
        <p className="bold">{blog.date}</p> <Link className="wt_btn duration-300 bold font-semibold hover:underline" href={`/${blog.title}`}>Read More</Link>
      </div>
    </div>
  );
};
    ...
    <section className="py-8 md:py-10 lg:py-14 xl:py-16 2xl:py-18">
      <Container>
        ...
        <div className="w-full flex flex-nowrap items-end gap-2">
          ...
          <h6 className="mb-4 heading">Our Newest Articles</h6>
        </div>
